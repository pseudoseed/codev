 ...
 *
 * ## What `ok` does NOT say, and why the drill cannot say it
 *
 ...
 * `shape-check-failed` beside it. The drill runs neither step, and neither
 * outcome was ever assigned anywhere in this file. The contract was described as
 * regenerated by a comment rather than by a generator — which is this project's
 * own recurring defect, a claim spelled the same way whether or not anything
 * checked it, sitting on the one tool whose subject is that distinction. So the
 * vocabulary above is now exactly the set this file can assign, and a test holds
 * the two together.
 *
 ...
 * the rebase. Regenerating from one therefore means moving the pin, which is the
 * adoption this drill exists in order NOT to perform. It is step 3 of the
 * procedure in `tools/t3-codegen/REFRESH.md`, taken when a rebase is adopted for
 * a reason.
 *
 * What the drill CAN answer without moving anything, and now does:
 *
 *   contractClosure.conflicted        do the generator's own inputs survive the
 *                                     merge? If not, regeneration is blocked
 *                                     before shape-check is even a question.
 *   contractClosure.sourceHash.moved  which closure files the rebased tree would
 *                                     hand the generator with DIFFERENT bytes
 *                                     than the vendored contract came from.
 *
 * The second is the generator's own load-bearing drift detector — `generate.mjs`
  ...
  return {
    ...
    detail: moved.length === 0
      ? 'every file the generator reads is byte-identical after the merge, so the rebased tree '
        + 'regenerates the contract already vendored. shape-check was NOT run — see '
        + 'contractRegeneration.'
      : `${moved.length} of ${pin.closure.length} closure files differ after the merge: `
        + `${moved.join(', ')}. The generator would emit a different contract from the rebased `
        + 'tree; how different is what classify-churn --upstream-movement answers, and what '
        + 'generate.mjs settles once a rebase is adopted.',
  };
}
 ...
 */
const CONTRACT_REGENERATION = {
  attempted: false,
  reason: 'generate.mjs refuses any checkout whose HEAD is not pin.commit, and a rebased tree '
    + 'never satisfies that — its head is a commit that did not exist before the rebase. '
    + 'Regenerating from one means moving the pin, which is the adoption this drill exists in '
    + 'order not to perform (step 3 of tools/t3-codegen/REFRESH.md). contractClosure.sourceHash '
    + 'bounds the job instead. shape-check did not run.',
};
...
if (target === base) {
  ...
  report({
    ...
    contractRegeneration: CONTRACT_REGENERATION,
  ...
  if (!rebase.ok) {
    ...
    result = {
      ...
      detail: `all ${carried} customization commits replayed onto ${target.slice(0, 12)} with no `
        + 'conflicts. That is what `ok` means here, and all it means — the contract was not '
        + 'regenerated and shape-check did not run.',
      rebasedHead,
