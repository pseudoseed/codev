...
A new package, `packages/porch-driver`, private like `t3-client` and depending on
it. Seven modules, each one deliverable-shaped:

- `harness-map.ts` — `--harness` to `driverKind`, `--model` to
  `modelSelection.model`, with three distinct refusals: unmapped, retired,
  model-unsupported.
- `commands.ts` — `commandId` generation and an fsynced append-only dispatch
  journal, written **before** dispatch, with `beforeDispatch` / `afterDispatch`
  hooks so a test can stand in either crash window.
- `cursor.ts` — the persisted cursor, advancing **after** the handler, with a
  `beforeAdvance` hook in the second window and a `reset` that moves backwards.
- `turn.ts` — settle detection on `activeTurnId` with the `seenRunning` latch,
  waiter registered before dispatch, role prompt as the first turn's content.
- `checks.ts` — `child_process` in the thread's `worktreePath`. No terminal RPC,
  as ruled.
- `worktree-setup.ts` — placement of the #1018 guard files and `opencode.json`.
- `thread.ts` — the object porch holds: create, run a turn, interrupt, run a
  check between turns.

86 → 143 tests in `packages/codev/src/__tests__/`, 57 of them new in
`spec-146-porch-driver.test.ts`.

## Three things I got wrong first and had to measure

**`subscribeThread` does not deliver `thread.meta-updated`.** I used
`thread.meta.update` as cheap traffic for the crash and idle scenarios, because it
costs no model call. It dispatched fine — the server returned
`{sequence: 3}` — and the subscription delivered nothing, so the first live run
reported "applied zero times" for a command that had applied. `ws.ts:293-312`
lists the six thread DETAIL events, and metadata is not among them. Every scenario
now uses a turn start as its traffic: it is delivered, and it is the command whose
double-application actually costs something.

That is the shape the lessons file warns about, arriving from the other side: the
evidence said "no" when the honest answer was "this subscription cannot see it".

**Node 20 cannot host the server.** `npx t3@0.0.35 serve` dies with
`No such built-in module: node:sqlite`. The harness needs node 22
(`~/.nvm/versions/node/v22.22.2`), which is also what Phase 2's evidence ran
under. Recorded here because the failure looks like a broken harness rather than a
wrong interpreter.

**The plan's test location does not run.** `packages/porch-driver/__tests__/` is
not reachable by `npm test`, which is `pnpm --filter @cluesmith/codev test`. The
tests live in `packages/codev/src/__tests__/spec-146-porch-driver.test.ts`, as
Phases 1 and 2 did, and the plan records the deviation.

## The finding: an interrupt stops the turn, not the work

Acceptance criterion 4 wanted an interrupted turn to settle **and** its command's
side effect to be absent. Half of it holds and half of it cannot.

Measured, three separate observations rather than one:

- the turn settles 34 ms after the interrupt, with `activeTurnId: null`
- the sentinel file is ABSENT at settle
- the sentinel file is PRESENT 30 seconds later

So the shell command `sleep 25; printf DONE > file` outlived the interrupt and
finished. t3code forwards the interrupt to the provider's own `turn/interrupt`
(`CodexSessionRuntime.ts:2150-2180`), and the process the provider had spawned
survived it. That is provider behaviour below t3code; no client change reaches it.

It is recorded as **not-demonstrated with a finding**, not as a pass and not as a
failure. The consequence porch has to hold: after an interrupt the thread is idle
while the worktree may still be being written, so "no turn is active" is not "the
tree is quiet". `runCheck`'s guard covers the active-turn case and cannot cover
this one.

## Evidence

`codev/research/146-phase3-live-evidence.json`, regenerated by
`codev/experiments/146-phase3-live/run.mjs` against a freshly created pinned
server. The crash scenarios kill a real child process with SIGKILL inside each
window — `crash-child.mjs`, which never catches the signal, because a signal a
process can catch is not the crash being tested.

## The full-suite green that was not green twice

`npm test` failed twice on two *different* pre-existing tests, and neither is
Phase 3's:

- run 1: `session-manager.test.ts` "no stderr tail logged for file-based stderr"
  timed out at 15s. It passes in isolation.
- run 2: `spawn-gate-profile.test.ts` ENOTEMPTY on teardown — the same race Phase 2
  fixed with a 250ms retry budget, back on a more loaded machine.

Both got a wider budget rather than a skip: 15s → 30s, and 250ms → 2s. No
assertion changed in either. Third run: 6,351 passed, 0 failed.

Recording the shape rather than the incident: **a fix sized to the machine that
found the bug is not a fix.** The 250ms retry passed on the machine where it was
written and failed on the machine that ran the suite under a live t3code server.

## Evidence discipline, kept from Phase 2

The first full harness run stamped `clientTreeDirty: true` and a `clientCommit`
...
# Phase 7, iteration 2 — the same defect, twice

Codex flagged in iteration 1 that the documented pairing request could not run: Tower's
shared-key check fired before `agent-auth.ts`, so the runbook's keyless POST returned 401.
I fixed exactly that request — one carve-out for `POST /api/agent/v1/pairing/redeem` — and
wrote a test proving the neighbours stayed refused.

Codex flagged in iteration 2 that a paired device still could not do anything. The
credential it now received was unusable, because every other route on the surface still
demanded the host key, and nothing puts that key on a remote device.

Same defect. The fix in between was real, and the test for it was real, and neither helped.

**What went wrong in my reasoning:** I fixed the request that was failing rather than the
flow the runbook describes. Redemption was the step that returned 401, so redemption is
what I unblocked. The step after it was never exercised keyless — the e2e sent
`towerHeaders()` on every post-pairing call — so nothing failed and the phase looked done.

The carve-out's own comment stated the bug in plain words and I read it as a design note:
*"Every other `/api/agent/v1/` route still requires the key AND a machine credential."*
That sentence describes a device that cannot use its credential. I wrote it, tested around
it, and shipped it.

**The rule:** a bootstrap is not done when the bootstrap request succeeds. It is done when
the thing it bootstraps can be used. The credential-only read is one line of test and it
is the only line that was ever load-bearing here.

**The fix** delegates the whole `/api/agent/v1/` prefix past the shared-key layer rather
than exempting one route. Not a removed check — the surface's own authentication is
strictly stronger than the key it stops requiring: per-machine, hash-stored, individually
revocable, none of which one shared secret can express. Safe wholesale because
`handleAgentRoute` claims the prefix and answers 404 itself, so nothing under it falls
through to a keyed handler. That property is asserted, not assumed.

Pinned from four sides, because a prefix check is the kind of thing that silently widens:
neighbouring paths (`/api/agent/v1-admin`, `/api/agent/v2/...`) are not swept in; the rest
of Tower still 401s keyless over the wire; every agent route still refuses with its own
named signal rather than Tower's bare `Unauthorized`; and `GET` on the pairing path is a
404 from the table, not an inherited exemption.

Also tested the SSE stream keyless, not just the one-shot read — a client lives on
`/stream`, and proving only `/state` would have left the route that matters unproven. It is
SSE over HTTP, so the delegation carries it; the WebSocket upgrade path still requires the
host key and would not have.

**Not demonstrated:** a successful revoke over HTTP. The route needs a human session, the
registry is in-memory and per-process, and the e2e Tower is a separate process with no
route to mint one. What is asserted is that the documented request reaches codev-agent and
is refused by name rather than dying at the shared-key layer. The other half is written
down as not done rather than covered by a store call dressed up as an HTTP test.

Pre-existing e2e failures in `cli-tower-mode`, `tower-reconnect`, `send-integration` and
`bugfix-1515-tower-isolation` were confirmed pre-existing by stashing the change and
