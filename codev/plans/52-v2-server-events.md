...
| Held mail flag | `heldSummaryForWorkspace` / `findHeldForAgent` eligibility (`not_before IS NULL OR not_before <= now`) | `listHeld` (includes pre-due rows) |
| Liveness + `lastDataAt` + `bytesWritten` | `getWorkspaceTerminals().get(ws)` then `getTerminalManager().getSession(id)`; public getters at `pty-session.ts:783` and `:825` | `getWorkspaceTerminalsEntry` (creates empty entries); editing PtySession |
| Workspace set | `getKnownWorkspacePaths()` minus `/.builders/` paths | Inventing an unregister write |
...
**Cursor.** Per-scope, process-wide. `snapshot` and `resumed` read it and do not increment it. `snapshot` is never written into the delta buffer. `resumed.seq` equals the `since` it honoured. Every other frame increments, then is appended to the buffer.
...
- [ ] `parentId`: workspace null; architect → workspace; builder → spawning architect if that architect node exists in the projection, else workspace
- [ ] `flags.heldMail` from eligible mailbox rows joined on `to_agent` (builder: `roleId`; architect: `architect.id`; workspace: false)
...
- [ ] Missing `scope` → 400. Malformed `since` → 400. Unknown path → 200 + `dark` naming that path's workspace id; other paths still snapshot
...
Route tests: reuse `makeReq` / `makeRes` / `makeCtx` from `tower-routes.test.ts`. Stub the bus and a snapshot projector. Cover the invalid-parameter table, cap, and two-path `dark`. Do not start a real Tower.
...
4. Loop every **100ms** while any v2 client is connected. `fs.watch` on `.builders/` wakes the loop immediately (spawn/cleanup). A timeout at the next mailbox `not_before` wakes `heldMail`.
5. Every 30s, one `tick` per live scope from `bytesWritten` deltas, then a status pass so `stalled` cannot wait longer than the spec's bound. The 100ms loop may emit `stalled` sooner; do not add a second delayed path.
6. Pause the loop when the last client disconnects. Keep cursors, `streamId`s, and buffers.

Default `V2Deps` bind to `discoverBuilders` (no filter), `getArchitects`, `heldSummaryForWorkspace`, `getWorkspaceTerminals().get` (never `getWorkspaceTerminalsEntry`), `getTerminalManager().getSession`, `getKnownWorkspacePaths`. Snapshot-on-connect may call `getRehydratedTerminalsEntry` once per workspace so the first frame is not stale after a Tower restart; the loop stays on the in-memory map.
...
|---|---|---|---|
| Calling `getOverview` on the 100ms loop hits GitHub | High if copied | High — idle CPU and flake | Call `discoverBuilders` only; state this in the module comment |
| Compare loop itself blows the idle *CPU* budget | Medium | Medium | Pause when no clients; watch-wake; keep the body to a map diff |
| `getWorkspaceTerminalsEntry` creates ghost workspaces | High if copied | Medium — false `offline` nodes | Use `Map.get` only |
| Workspace `gone` has no production write | Certain | Low — `offline` covers deactivate | Injected `listWorkspaces`; do not invent unregister |
