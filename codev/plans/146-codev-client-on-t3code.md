...
- `packages/porch-driver/src/checks.ts` — **added, not planned.** `runPhaseCheck` and the
  active-turn guard. The deliverable put phase checks in this phase without naming a file; they
  are their own concern (a process, a shell, a timeout that is not a failure) and folding them
  into `thread.ts` would have hidden that.
- ~~`packages/porch-driver/__tests__/`~~ → `packages/codev/src/__tests__/spec-146-porch-driver.test.ts`.
  **Deviation.** The check porch runs is `npm test`, which is
  `pnpm --filter @cluesmith/codev test`; a test under `packages/porch-driver/` would look present
  and never run. Phases 1 and 2 landed in the same place for the same reason.
- `codev/experiments/146-phase3-live/` — **added.** The live harness (`run.mjs`, `crash-child.mjs`,
  `lib.mjs`) that discharges the acceptance criteria and the exit conditions against a real pinned
  server, and writes `codev/research/146-phase3-live-evidence.json`. Phase 2's equivalent was
  ephemeral, so its evidence could not be regenerated by anyone but the session that made it.
- `package.json` (root) — `porch-driver` added to the build chain, so `npm run build` typechecks it.

#### Deliverables

- [x] Settle detection keys on `activeTurnId: null`, not on session status. The spec records that
      an interrupted turn reports status `ready`, so status alone cannot distinguish an interrupted
      turn from a finished one.

      `turn.ts`. The `seenRunning` latch is load-bearing, not defensive: `activeTurnId` is already
      null in the `thread.session-set` emitted at thread creation, so a detector without it reports
      settled before the turn starts. Live evidence shows an interrupted turn reporting
      `status: "ready"` with `activeTurnId: null` — the two states status cannot separate.
- [x] Every command carries a porch-generated `commandId`, journalled to disk **before** dispatch.

      `commands.ts`, `write(2)` + `fsync(2)` per record. A record still in the page cache when the
      machine dies did not survive the crash the journal exists for.
- [x] The sequence cursor advances **after** the handler completes, never before. Every handler is
...
- [x] Phase checks run as shell in the thread's own `worktreePath`, outside the thread, between
      ...
      **Ruled by the architect: not through a t3code terminal.** porch spawns the check process
      ...
      Done in `checks.ts`. `pin.json` is unchanged. A check refuses to run while a turn is active
      (`TurnActiveError`), and a timeout reports `timedOut: true` with `exitCode: null` rather than
      a number a caller could read as a verdict. The interpreter is recorded on every result,
      because Phase 2 lost half a day to a test that failed under bash and passed under zsh.

      **Every blocking finding after iteration 1 was a timeout that did not bound what it named:**
      the check timeout signalling the shell rather than the group and resolving on `close`;
      `runTurn` holding the full budget twice; the SIGKILL escalation cancelled at the moment it
      was needed; and the turn budget starting after the dispatch, leaving a hung dispatch
      unbounded. Each had been tested against the input where the thing it bounds is trivially
      bounded anyway — an `exec`ing shell, an instant dispatcher, a child that dies on SIGTERM.
      **A timeout test whose subject would finish on its own is not a timeout test.**
- [x] `--harness` maps to a t3code `driverKind` and `--model` to `modelSelection.model`. An
      unsupported pair fails at spawn, matching today's `assertHarnessAcceptsModel` behaviour.

      `harness-map.ts`, with three refusals that are deliberately not the same error: unmapped
      (no driver for this name), retired (the name was real and Codev dropped it), and
      model-unsupported. `claude` maps to `claudeAgent`; two of the five kinds match Codev's names
      by accident, which is what makes a table look unnecessary until the third one fails silently.
      The model-accepting and retired sets are duplicated rather than imported — porch-driver must
      not depend on `agent-farm` — and a test asserts each against the real table.
...
- [x] `worktree-setup.ts` reimplements the Claude write-guard from #1018 and `opencode.json`
      ...
      **Placement is reimplemented; the guard's CONTENT is injected.** `buildWorktreeGuardFiles`
      owns the hook script and the `.claude/settings.local.json` it shares with the phase
      stop-guard; copying it here would be a second copy that drifts. A caller that supplies none
      gets `guard: 'absent'` with a reason — never a silent skip, and never the same value as
      `'not-applicable'`.

      **Does the guard still fire under t3code's Claude driver?** Verified by reading, not by
      running: `ClaudeAdapter.ts:1243-1247` and `:4312` at the pinned commit pass
      `settingSources: ["user", "project", "local"]` to the Agent SDK, and `local` is the file the
...
- [x] Tests for this phase. 82 in `packages/codev/src/__tests__/spec-146-porch-driver.test.ts`,
      41 of them mutation-checked by `codev/research/146-phase3-mutation-check.py`: each reverts
      one property and asserts the test that names it goes red. All 41 do.
...
- [x] A thread is created on a worktree, runs a turn to settle, has a file written externally in
      its `worktreePath`, and reads the new value back on the next turn. Live scenario A. The
      external write goes through the phase-check path, so it also demonstrates a check running
      between turns as a process porch owns.
- [x] Killing the driver between journal write and dispatch, then restarting, does not apply the
      command twice. Live scenario E, with a real child process `SIGKILL`ed in the window and
      nothing catching the signal. **Both** windows are covered: killed before the dispatch (the
      command never landed, recovery applies it once) and killed after it (the command landed,
      recovery re-dispatches the same `commandId` and t3code's receipt table collapses it —
      `OrchestrationEngine.ts:142-169`). One application observed in each case.
- [x] Killing the driver inside the window between handler completion and cursor write causes the
      event to be **reprocessed**, not skipped. The test asserts reprocessing, since that is the
      property the at-least-once choice buys. Live scenario F: sequence 43 applied, process
      `SIGKILL`ed before the cursor moved, cursor still at 42 on restart, 43 applied again.
- [~] A turn interrupted mid-shell reports `activeTurnId: null` — **demonstrated**. The
      interrupted command's side effect is absent — **not demonstrated, with a finding.**

      Measured as three observations rather than one: the turn settles 34 ms after the interrupt
      with `activeTurnId: null`; the sentinel file is ABSENT at settle; the sentinel file is
      PRESENT 30 seconds later. The shell command outlived the interrupt and finished.

      t3code forwards the interrupt to the provider's own `turn/interrupt`
      (`CodexSessionRuntime.ts:2150-2180`), and the process the provider spawned survived it. That
      is provider behaviour below t3code; no client change reaches it. Recorded as
      `not-demonstrated` with the finding rather than as a pass or a failure.
...
- [x] Build and tests pass. `npm run build` clean; `npm test` 6,376 passed, 48 skipped, 0 failed
...
- [x] **Phase 2's criterion C, discharged here.** A socket killed mid-stream at a known
      ...
      **Done that way.** Live scenario C holds a control subscription that never drops, kills a
      second one mid-stream at a known server-issued sequence while a real turn is running, and
      compares the replayed `eventId` list against the control's record over the same window. Seven
      events in the gap, lists equal, completion event included.
...
- [x] **The stream timeout is proven idle, not total, against a real long-lived subscription.**
      ...
      **Done.** Live scenario D held one `ResumingSubscription` for 361 seconds under continuous
      traffic — 17 turns, 150 handler calls — on **one** subscription attempt, with no timeout and
      no resubscription. The traffic has to be thread DETAIL events: a `thread.meta.update` is not
      one (`ws.ts:293-312`), so a heartbeat built from metadata edits would have left the stream
      silent while looking busy, and the timeout would then have fired legitimately against the
      wrong hypothesis. That was the first version of this scenario, and it is why the harness uses
      turns.
