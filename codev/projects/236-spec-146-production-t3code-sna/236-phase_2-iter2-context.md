### Iteration 1 Reviews
- claude: REQUEST_CHANGES — Cache design and wiring are correct, but two states report a word that is false — a ready backend with no threads says `connecting` forever, and an unobserved thread is published as `available`.
- opencode: REQUEST_CHANGES — Placeholder cache rows make snapshot lie `available` before any observation, and a ready backend with no thread ids lies `connecting` forever.
