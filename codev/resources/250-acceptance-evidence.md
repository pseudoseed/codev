...
| the contract regenerates and passes `shape-check` | **not run on the rebased tree.** The closure merges without conflict, so regeneration is not blocked; 4 of its 9 files come out changed, so the result would differ from the vendored contract |
...
those terms rather than as a bare "met". Under the literal reading it is not, and no wording here
should let a reader arrive at "met" without seeing which of the two they are being handed.
...
- `contractRegeneration.attempted` is `false` in every result of a drill that **ran** — `ok` and
  `conflicts` — with the reason above. `could-not-run` carries no measurement-shaped fields at
  all, deliberately: it means nothing was learned, and a field on it is the first thing a reader
  would mistake for a finding;
